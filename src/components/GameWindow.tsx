...
const GameWindow: React.FC<GameWindowProps> = props => {
    ...
    function handleClick(row: number, col: number) {
        ...
        return () => {
            ...
            buttonsRef.current[row + col * 10].style.border = 'dashed 1px';
            buttonsRef.current[row + col * 10].style.borderTop = 'none';
            buttonsRef.current[row + col * 10].style.borderLeft = 'none';
            ...
            for (let i = -1; i < 2; i++) {
                for (let j = -1; j < 2; j++) {
                    if ((row + i) < 0 || (row + i) > 8) continue;
                    if ((col + j) < 0 || (col + j) > 8) continue;
    ...
    return(
        <div className="game-window">
            {props.gameMatrix.map((row: [], col: number) => (
